...
void init() {
    ...
    //make kernel_source
    kernel_source = get_source_code("skernel.cl",&kernel_source_size);
    CHECK_ERROR(err);
...
void recoverVideo(unsigned char *videoR, unsigned char *videoG, unsigned char *videoB, int *vrIdx, int N, int H, int W) {
    ...
    size_t global_size = N*(N-1)/2;
    size_t local_size = 16;

    global_size = (global_size + local_size - 1) / local_size * local_size;

    err = clEnqueueNDRangeKernel(queue[0], kernel, 1, NULL, &global_size, &local_size, 0, NULL, NULL);
    CHECK_ERROR(err);
